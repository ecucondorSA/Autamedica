...
function assertClientEnvAllowed(name) {
    if (!ALLOWED_CLIENT_VARS.has(name)) {
        throw new Error(`Invalid client environment variable: ${name}. Only approved NEXT_PUBLIC_* variables are allowed on client side.`);
    }
    if (SERVER_ONLY_VARS.has(name.replace("NEXT_PUBLIC_", ""))) {
        throw new Error(`Security violation: ${name} appears to be a server-only variable exposed to client`);
    }
}
// Utilidad específica para variables de entorno del cliente (NEXT_PUBLIC_*)
export function ensureClientEnv(name) {
...
export function getClientEnvOrDefault(name, defaultValue) {
    assertClientEnvAllowed(name);
    const value = process.env[name];
    return value ?? defaultValue;
}
export function getOptionalClientEnv(name) {
    assertClientEnvAllowed(name);
    return process.env[name] ?? undefined;
}
// Utilidad específica para variables de entorno del servidor (sin NEXT_PUBLIC_)
export function ensureServerEnv(name) {
    if (name.startsWith("NEXT_PUBLIC_")) {
        throw new Error(`Invalid server environment variable: ${name}. Server-side code should not use NEXT_PUBLIC_ variables directly.`);
    }
    const value = process.env[name];
    if (!value) {
...
export function getServerEnvOrDefault(name, defaultValue) {
    if (name.startsWith("NEXT_PUBLIC_")) {
        throw new Error(`Invalid server environment variable: ${name}. Server-side code should not use NEXT_PUBLIC_ variables directly.`);
    }
    const value = process.env[name];
...
export function validateProductionEnvironment() {
    ...
    const payments = paymentVars.every((varName) => {
        const value = process.env[varName];
        if (!value) {
            issues.push(`Missing payment variable: ${varName} (required for production)`);
            return false;
        }
        return true;
    });
...
export function validateEnvironmentSecurity() {
    ...
    for (const serverVar of Array.from(SERVER_ONLY_VARS)) {
        ...
        if (process.env[exposedVar]) {
            throw new Error(`Security violation: Server-only variable ${serverVar} is exposed as ${exposedVar}`);
        }
    }
...
export function validateStagingEnvironment() {
    ...
    return {
        ...
        issues: [
            ...issues,
            ...prodValidation.issues.filter((issue) => !issue.includes("MercadoPago")),
        ],
    };
}
export function validateEnvironmentByType(envType) {
