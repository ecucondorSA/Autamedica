...
export async function seedCompanyExample() {
  logger.info('Seeding company example...');
  ...
  try {
    ...
    logger.info(`Created auth user: ${adminAuthData.user.email}`);
    ...
    logger.info(`Created auth user: ${memberAuthData.user.email}`);
    ...
    logger.info('Inserted user profiles.');
    ...
    logger.info('Inserted company members.');

    logger.info('✅ Company example seeded successfully!');

  } catch (error) {
    logger.error('Error seeding company example:', error);
