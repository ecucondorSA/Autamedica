-- ==========================================
-- SUPABASE SCHEMA - COMPLETE SETUP
-- ==========================================
-- Schema completo incluyendo tablas base necesarias
-- Ejecutar en orden: Base Tables → Patient Portal Tables
...
-- ==========================================
-- BASE TABLES (Si no existen)
-- ==========================================

-- Appointments table (requerida para telemedicine)
CREATE TABLE IF NOT EXISTS appointments (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  patient_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  doctor_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  appointment_type TEXT NOT NULL DEFAULT 'consultation' CHECK (
    appointment_type IN ('consultation', 'followup', 'telemedicine', 'emergency')
  ),
...
-- Patient care team (requerida para RLS de anamnesis)
CREATE TABLE IF NOT EXISTS patient_care_team (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  patient_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  doctor_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  role TEXT NOT NULL CHECK (role IN ('primary_doctor', 'specialist', 'consultant')),
  specialty TEXT,
...
CREATE INDEX IF NOT EXISTS idx_patient_care_team_patient_id ON patient_care_team(patient_id);
CREATE INDEX IF NOT EXISTS idx_patient_care_team_doctor_id ON patient_care_team(doctor_id);
...
CREATE POLICY "Users can view care team" ON patient_care_team
  FOR SELECT USING (auth.uid() IN (patient_id, doctor_id));

-- ==========================================
-- PATIENT PORTAL TABLES
-- ==========================================
-- Ahora incluye el contenido de supabase-schema-patients.sql
-- ==========================================
