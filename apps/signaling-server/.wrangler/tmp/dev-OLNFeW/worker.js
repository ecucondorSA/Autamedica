  ...
  async handleJoin(request, corsHeaders) {
    ...
    if (!this.rooms.has(roomId)) {
      this.rooms.set(roomId, {
        ...
        // Store pending calls for patients
      });
    }
    ...
    if (userType === "patient" && room.pendingCalls) {
      ...
      if (pendingCall) {
        ...
        room.messages.push({
          ...pendingCall,
          timestamp: Date.now()
          // Update timestamp to ensure it's received
        });
      }
    }
...
// .wrangler/tmp/bundle-rWbChX/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-rWbChX/middleware-loader.entry.ts
