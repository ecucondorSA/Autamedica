...
export default function RegisterForm() {
  ...
  const handleEmailRegistration = async (e: React.FormEvent) => {
    ...
    if (isDummyMode) {
      // Simulate registration in development mode
      setTimeout(() => {
        setError(null)
    ...
    try {
      ...
      router.push('/auth/login?message=Cuenta creada exitosamente. Revisa tu email.')
    } catch (err) {
  ...
  const handleOAuthRegistration = async (provider: 'google' | 'github') => {
    setLoading(true)
    setError(null)
    ...
    if (isDummyMode) {
      // Simulate OAuth registration in development mode
      setTimeout(() => {
        setError(null)
        router.push('/auth/select-role') // Redirect to role selection
        setLoading(false)
      }, 1000)
      return
    }
  ...
  return (
    <>
      ...
      <form className="space-y-6" onSubmit={handleEmailRegistration}>
      ...
      <div className="space-y-3">
        ...
        >
          ...
          {loading ? 'Conectando...' : 'Registrarse con GitHub'}
        </button>
      </div>
