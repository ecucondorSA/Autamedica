...
import { getPortalForRole, safeRedirectOrFallback } from '@autamedica/shared/auth/portals';
import type { UserRole } from '@autamedica/shared/auth/portals';
  ...
  if (code) {
    try {
      ...
      if (!profile || !profile.role) {
        // No role assigned yet, redirect to role selection page
        console.log(`User ${user.email} has no role, redirecting to role selection`);
        return NextResponse.redirect(
      ...
      const defaultDestination = getPortalForRole(finalRole);
      const destination = safeRedirectOrFallback(returnTo, defaultDestination);
