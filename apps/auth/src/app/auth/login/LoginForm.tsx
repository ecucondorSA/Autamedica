...
export function LoginForm() {
  ...
  const { loading, error: authError, isAuthenticated } = useAuthContext();
  ...
  useEffect(() => {
    if (isAuthenticated && role) {
      const targetUrl = returnTo || `/auth/callback?role=${role}`;
      router.push(targetUrl);
    }
  }, [isAuthenticated, role, returnTo, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!result.success) {
      setLocalError(result.error?.message || 'Error al iniciar sesión');
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (!result.success) {
      setLocalError(result.error?.message || 'Error con Google');
      setIsSubmitting(false);
    }
    // OAuth redirect happens automatically, no need to handle success
  };

  // Don't render until we have a valid role
      ...
      <div className="w-full max-w-md">
        <div className="bg-[var(--au-surface)] border-2 border-[var(--au-border)] rounded-2xl shadow-2xl p-8">
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 px-6 py-3 bg-[var(--au-hover)] hover:bg-[var(--au-hover-border)] border-2 border-[var(--au-border)] hover:border-[var(--au-accent)] rounded-xl text-[var(--au-text-primary)] font-medium transition-all duration-200 mb-6 disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                required
                disabled={isLoading}
                className="w-full px-4 py-3 bg-[var(--au-hover)] border-2 border-[var(--au-border)] rounded-xl text-[var(--au-text-primary)] placeholder:text-[var(--au-text-tertiary)] focus:outline-none focus:ring-2 focus:ring-[var(--au-accent)] focus:border-transparent transition-all disabled:opacity-50"
