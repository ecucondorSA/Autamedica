      ...
      case 'number':
        return (
          <input
            ...
            className="w-full px-4 py-3 border-2 border-stone-300 rounded-lg focus:border-stone-700 focus:ring-2 focus:ring-stone-200 outline-none transition-all text-stone-900"
          />
        );
      ...
      case 'textarea':
        return (
          <textarea
            ...
            required={field.required}
            rows={4}
            className="w-full px-4 py-3 border-2 border-stone-300 rounded-lg focus:border-stone-700 focus:ring-2 focus:ring-stone-200 outline-none transition-all text-stone-900 resize-none"
          />
        );
              ...
              return (
                ...
                >
                  <input
                    ...
                    className="mt-1 h-5 w-5 text-stone-700 border-stone-300 rounded focus:ring-stone-500"
                  />
                  <div className="flex-1">
                    <p className="font-semibold text-stone-900">{option.label}</p>
                    {option.explanation && (
                      <p className="text-sm text-stone-600 mt-1">{option.explanation}</p>
                    )}
                  </div>
                </label>
              );
      ...
      case 'boolean':
        return (
          <div className="flex gap-4">
            <label className={`flex-1 p-4 border-2 rounded-lg cursor-pointer transition-all text-center ${
              ...
              <input
                ...
                className="sr-only"
              />
              <span className="font-semibold text-stone-900">✓ Sí</span>
            </label>
            <label className={`flex-1 p-4 border-2 rounded-lg cursor-pointer transition-all text-center ${
        ...
        return (
          <div className="space-y-4">
            <input
              ...
              className="w-full h-3 bg-stone-200 rounded-lg appearance-none cursor-pointer accent-stone-700"
            />
            <div className="flex justify-between text-sm text-stone-600">
              <span>1 (Leve)</span>
              <span className="text-2xl font-bold text-stone-900">{value || field.validation?.min || 1}</span>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-start justify-between gap-2">
        <label className="text-label text-stone-900 flex-1">
          {field.label}
        ...
        {field.tooltip && (
          <div className="relative">
            ...
            {showTooltip && (
              <div className="absolute right-0 top-8 z-10 w-64 p-3 bg-stone-900 text-white text-sm rounded-lg shadow-xl">
                <p className="whitespace-pre-line">{field.tooltip}</p>
                <div className="absolute -top-2 right-2 w-0 h-0 border-l-8 border-r-8 border-b-8 border-transparent border-b-stone-900"></div>
              </div>
            )}
          </div>
        )}
